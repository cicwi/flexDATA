...
def init_meta(geometry=None):
    ...
    description = {"name": "n/a", "comments": "n/a", "date": "n/a", "duration": "n/a"}
...
def init_geometry(
    ...
    if geom_type == GEOM_STAOFF:
        # Source position:
        geometry["src_vrt"] = 0.0
        geometry["src_hrz"] = 0.0
        geometry[
            "src_mag"
        ] = (
            0.0
        )  # This value should most of the times be zero since the SOD and SDD distances are known

        # Detector position:
        geometry["det_vrt"] = 0.0
        geometry["det_hrz"] = 0.0
        geometry["det_mag"] = 0.0  # same here
        geometry["det_rot"] = 0.0

        # Axis of rotation position:
        geometry["axs_hrz"] = 0.0
        geometry["axs_mag"] = 0.0  # same here
...
def read_tiffs(
    path, name, skip=1, sample=1, x_roi=[], y_roi=[], dtype="float32", memmap=None
):
    """
    ...
    if len(files) == 0:
        raise IOError("Tiff files not found at:", os.path.join(path, name))
    ...
    if memmap:
        data = numpy.memmap(
            memmap, dtype=dtype, mode="w+", shape=(file_n, sz[0], sz[1])
        )
    ...
    time.sleep(
        0.1
    )  # This is needed to let print message be printed before the next porogress bar is created
    ...
    print(
        "%u files were loaded. %u%% memory left (%u GB)."
        % (success, free_memory(True), free_memory(False))
    )
    time.sleep(
        0.1
    )  # This is needed to let print message be printed before the next porogress bar is created
...
def astra_proj_geom(geometry, data_shape, index=None):
    ...
    if geometry.get("_thetas_") is not None:
        ...
        if len(thetas) != theta_count:
            raise IndexError(
                "Length of the _thetas_ array doesn`t match withthe number of projections: %u v.s. %u"
                % (len(thetas), theta_count)
            )
    else:

        thetas = (
            numpy.linspace(
                geometry.get("theta_min"),
                geometry.get("theta_max"),
                theta_count,
                dtype="float32",
            )
            / 180
...
def _sanity_check_(meta):
    ...
    for word in minimum_set:
        if word not in meta["geometry"]:
            raise ValueError(
                "Missing "
                + word
                + " in the meta data. Something wrong with the log file?"
            )
...
def _file_to_dictionary_(path, file_mask, separator=":"):
    ...
    log_file = [
        ...
        if (
            os.path.isfile(os.path.join(path, x)) and file_mask in os.path.join(path, x)
        )
    ]
    ...
    if len(log_file) == 0:
        ...
        raise Exception(
            "Log file not found @ " + os.path.join(path, "*" + file_mask + "*")
        )
        # return None
...
def _parse_unit_(string):
    ...
    factor = [
        units_dictionary[key]
        for key in units_dictionary.keys()
...
def _flexray_translate_(records):
    ...
    geometry["det_pixel"] = numpy.round(
        geometry["img_pixel"] * (geometry["src2det"] / geometry["src2obj"]), 5
    )
...
def _flex_motor_correct_(geometry, settings):
    ...
    geometry["vol_tra"][0] = (
        geometry["det_vrt"] * geometry["src2obj"]
        + geometry["src_vrt"] * geometry["det2obj"]
    ) / geometry.get("src2det")
...
def _modify_astra_vector_(proj_geom, geometry):
    ...
    for ii in range(0, theta_count):
        ...
        # Use linear offsets:
            ...
            raise ValueError("Wrong geometry type: " + geometry.get("type"))
        ...
        T = numpy.array(
            [
                geometry["vol_tra"][1] * vect_norm / det_pixel[1],
                geometry["vol_tra"][2] * vect_norm / det_pixel[1],
                geometry["vol_tra"][0] * vect_norm / det_pixel[0],
            ]
        )
...
def _metadata_translate_(records):
    ...
    descr_dict = {"duration": "total_scantime", "name": "scan_name"}
    ...
    geometry["img_pixel"] = geometry["det_pixel"] / (
        geometry["src2det"] / geometry["src2obj"]
    )

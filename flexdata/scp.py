...
def ssh_get_path(
    local_path, remote_path, hostname, username, password=None, ovewrite="different"
):
    """
    ...
    # Connect to remote:
    sftp = connect_sftp(
        hostname, username, password, os.path.join(local_path, "scp.log")
    )
...
class _MySFTPClient_(paramiko.SFTPClient):
    ...
    def _put_path_(self, local, remote, overwrite):
        ...
        for item in os.listdir(local):
            if os.path.isfile(os.path.join(local, item)):
                ...
                if self._overwrite_(
                    os.path.join(local, item), os.path.join(remote, item), overwrite
                ):
    ...
    def put_path(self, local, remote, overwrite="different"):
        ...
        for root, subdirs, files in os.walk(local):
            self._total_file_count_ += len(files)
    ...
    def _get_path_(self, local, remote, overwrite):
        ...
        for filename in self.listdir(remote):
            ...
            if stat.S_ISDIR(self.stat(os.path.join(remote, filename)).st_mode):
                ...
                self._get_path_(
                    os.path.join(local, filename), os.path.join(remote, filename)
                )
                ...
                if self._overwrite_(
                    ...
                    self.get(
                        os.path.join(remote, filename), os.path.join(local, filename)
                    )
    ...
    def get_path(self, local, remote="different"):
        ...
        print("Counting files...")
        self._total_file_count_ = 0
        for root, subdirs, files in self.sftp_walk(remote):
            self._total_file_count_ += len(files)

...
def crop(array, dim, width, geometry=None):
    ...
    if geometry:
        shift_geometry(geometry, h / 2, v / 2)
...
def rewrite_memmap(old_array, new_array):
    ...
    if isinstance(old_array, numpy.memmap):
        ...
        old_array = numpy.memmap(
            old_array.filename, dtype="float32", mode="r+", shape=new_array.shape
        )
        old_array[:] = new_array[:]
...
def tiles_shape(shape, geometry_list):
    ...
    # Big slice:
    new_shape = numpy.array(
        [(max_y - min_y) / det_pixel, shape[1], (max_x - min_x) / det_pixel]
    )
